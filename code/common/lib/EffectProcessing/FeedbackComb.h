#ifndef EFFECT_PROCESSING_FEEDBACK_COMB_H
#define EFFECT_PROCESSING_FEEDBACK_COMB_H
...
template<uint32_t BLOCK_SIZE, uint32_t DELAY>
void FeedbackComb<BLOCK_SIZE, DELAY>::update(float gain)
{
    m_gain = gain;
}

template<uint32_t BLOCK_SIZE, uint32_t DELAY>
float* FeedbackComb<BLOCK_SIZE, DELAY>::process(float* input)
{
    for (uint32_t i = 0; i < BLOCK_SIZE;)
    {
        UNROLL_8(
            m_state[m_currentIndex] = input[i] + m_gain * m_state[m_outputIndex];
            m_output[i] = m_state[m_outputIndex];

    ...
    m_currentDownOctaveVolumeLevel(1), m_currentUpOctaveVolumeLevel(1)
{
    ...
    designSecondOrderButterworthLowPassFilter(filterCoefficients, OCTAVE_DOWN_BAND_PASS_FC_2, samplingFrequency);
    ...
    memcpy(m_data + 2 * sizeof(BiquadCoefficients), &filterCoefficients, sizeof(BiquadCoefficients));
...
void OctaverDesigner::update(uint8_t downOctaveVolumeLevel, uint8_t upOctaveVolumeLevel)
{
    ...
    memcpy(m_data + 3 * sizeof(BiquadCoefficients), &downOctaveVolume, sizeof(float));
    memcpy(m_data + 3 * sizeof(BiquadCoefficients) + sizeof(float), &upOctaveVolume, sizeof(float));
}

...
static void test_ContourDesigner_getters()
{
    ...
    TEST_ASSERT_EQUAL(44, designer.dataSize());
...
static void test_ContourDesigner_update()
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1.97398993, data[3]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, -0.97415906, data[4]);

    // High shelving filter
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1.83352962, data[5]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, -2.89953989, data[6]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1.14633157, data[7]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1.43317973, data[8]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, -0.51350104, data[9]);

    // Global gain
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.5012, data[10]);

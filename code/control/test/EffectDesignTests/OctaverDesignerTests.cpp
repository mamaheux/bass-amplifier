...
static void test_OctaverDesigner_getters()
{
    ...
    TEST_ASSERT_EQUAL(56, designer.dataSize());
...
static void test_OctaverDesigner_update()
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, -1.981489, data[4]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.9816583, data[5]);

    // Low pass filter
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.003916144, data[6]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.007832288, data[7]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.003916144, data[8]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1, data[9]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, -1.815341, data[10]);
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.8310056, data[11]);

    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 0.5019, data[12]); // Octave down volume
    TEST_ASSERT_FLOAT_WITHIN(MAX_ABS_ERROR, 1, data[13]); // Octave up volume
